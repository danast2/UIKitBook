...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func sceneWillResignActive(_ scene: UIScene) {
        // Принудительное сохранение задач при уходе в фоновый режим
        guard let taskListController = window?.rootViewController as? TaskListController else {
            return
        }
        let allTasks = taskListController.tasks.flatMap { $0.value }
        taskListController.tasksStorage.saveTasks(allTasks)
    }
